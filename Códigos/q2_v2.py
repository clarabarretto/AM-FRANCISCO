...
def learning_curve_f1(
    ...
    for ax, (scores, label) in zip(axes, [(train_scores, "Treino"), (test_scores, "Teste")]):
        for name, color, marker in zip(model_names, colors, markers):
            ax.plot(proportions * 100, scores[name], marker=marker, markersize=6, 
...
# ============================================================
def print_latex_table(ci_A, ci_B):
    print("\n" + "="*70)
    print("CÓDIGO LATEX DA TABELA PRONTO (COPIE E COLE NO OVERLEAF):")
    print("="*70 + "\n")
...
"""
    ...
    nomes_latex = {
        "BayesGauss": "Bayes Gaussiano  ",
        "kNN": "$k$-NN Bayesiano ",
        "Parzen": "Parzen           ",
        "LogReg": "Regressão Log.   ",
        "Voting": "Voto Majoritário "
    }
    
    dfA = ci_A[ci_A["Métrica"] == "f1"].set_index("Modelo")
    dfB = ci_B[ci_B["Métrica"] == "f1"].set_index("Modelo")
    
    for mod in ["BayesGauss", "kNN", "Parzen", "LogReg", "Voting"]:
        mA, infA, supA = dfA.loc[mod, "Média"], dfA.loc[mod, "IC_inf"], dfA.loc[mod, "IC_sup"]
        mB, infB, supB = dfB.loc[mod, "Média"], dfB.loc[mod, "IC_inf"], dfB.loc[mod, "IC_sup"]
        linha = f"        {nomes_latex[mod]} & {mA:.3f} & [{infA:.3f}, {supA:.3f}] & {mB:.3f} & [{infB:.3f}, {supB:.3f}] \\\\"
        latex_code += linha + "\n"
